...
def _get_brz_log_filename():
    """Compute the default brz log filename.

    :return: A path to the would-be default log file (BRZ_LOG env, or a path
...
def _open_brz_log():
    """Open the brz log file, writing a warning to stderr if it cannot be opened.

    :return: An open file handle, or None if the log could not be opened.
    """
    import os
    import sys
    ...
    filename = None
    try:
        filename = _get_brz_log_filename()
        # Try opening directly so we control the error reporting destination
        # (sys.stderr is mockable; the Rust handler writes to a different fd).
        fd = os.open(filename, os.O_WRONLY | os.O_CREAT | os.O_APPEND, 0o644)
        os.close(fd)
    except OSError as e:
        path = e.filename if e.filename else filename
        sys.stderr.write(f"failed to open trace file: {e!s}: {path!r}\n")
        return None
    try:
        return _cmd_rs.open_brz_log()
    except OSError as e:
        path = e.filename if e.filename else filename
        sys.stderr.write(f"failed to open trace file: {e!s}: {path!r}\n")
        return None
