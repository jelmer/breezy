...
"""
            try:
                ...
                if (
                    first_error["type"] == "UNPROCESSABLE"
                    and first_error["path"] == "enablePullRequestAutoMerge"
                ):
...
class GitHub(Forge):
    ...
    def _get_repo_pulls(self, path, head=None, state=None):
        ...
        if state is not None:
            params["state"] = state
        path += ";".join(
            ["{}={}".format(k, urlutils.quote(v)) for k, v in params.items()]
        )
        response = self._api_request("GET", path)
        if response.status == 404:
            raise NoSuchProject(path)
    ...
    def get_derived_branch(
        ...
        try:
            ...
            raise errors.NotBranchError(
                "{}/{}/{}".format(WEB_GITHUB_URL, owner, project)
            )
        if preferred_schemes is None:
            preferred_schemes = DEFAULT_PREFERRED_SCHEMES
        for scheme in preferred_schemes:
