...
class TestVersionFileSource(TestCaseWithTransport):
    ...
    def test_file_id_wins_over_source(self):
        ...
        with dest.preview_transform() as tt:
            ...
            tt.create_file([b"x"], foo_tid)
            tt.version_file(
                foo_tid, file_id=b"explicit-id", source=(source, "foo")
            )
            self.assertEqual(b"explicit-id", tt.final_file_id(foo_tid))

    def test_no_file_id_and_no_source_fabricates(self):
        """With neither argument the implementation must still succeed
        by fabricating an id from the final path.
        """
        dest = self.make_branch_and_tree("dest")
        self.build_tree(["dest/foo"])
        with dest.preview_transform() as tt:
            foo_tid = tt.trans_id_tree_path("foo")
            tt.create_file([b"x"], foo_tid)
            tt.version_file(foo_tid)
            self.assertIsNotNone(tt.final_file_id(foo_tid))
...
class TestBuildTree(TestCaseWithTransport):
    ...
    def test_build_with_references(self):
        ...
        subtree = self.make_branch_and_tree(
            "source/subtree", format="development-subtree"
        )
    ...
    def test_file_conflict_handling(self):
        ...
        self.assertEqual(
            [
                DuplicateEntry(
                    "Moved existing file to", "file.moved", "file", None, "new-file"
                )
            ],
            target.conflicts(),
        )
    ...
    def test_directory_conflict_handling(self):
        ...
        self.assertEqual(
            [DuplicateEntry("Diverted to", "dir1.diverted", "dir1", "new-dir1", None)],
            target.conflicts(),
        )
        ...
        self.assertEqual(
            [
                DuplicateEntry(
                    "Diverted to", "dir1/file.diverted", "dir1/file", "new-file", None
                )
            ],
            target.conflicts(),
        )
    ...
    def test_mixed_conflict_handling(self):
        ...
        self.assertEqual(
            [
                DuplicateEntry(
                    "Moved existing file to", "name.moved", "name", None, "new-name"
                )
            ],
            target.conflicts(),
        )
    ...
    def test_raises_in_populated(self):
        ...
        target.add("name")
        self.assertRaises(
            errors.WorkingTreeAlreadyPopulated, build_tree, source.basis_tree(), target
        )
    ...
    def test_build_tree_observes_sha(self):
        ...
        source.add(
            ["file1", "dir", "dir/file2"], ids=[b"file1-id", b"dir-id", b"file2-id"]
        )
