...
class Merge3Merger:
    ...
    def _entries3(self):
        ...
        Drives off ``iter_changes(other vs base, extra_trees=[this])`` and
        derives per-tree names/parents purely from paths, so any tree that
        implements ``iter_changes`` works uniformly regardless of whether it
        carries file ids.
    ...
    def _entries_lca(self):
        ...
        this_path), ((base_parent_path, [lca_parent_paths]),
        other_parent_path, this_parent_path), ...)``. Parent slots are
        directory paths (``""`` = tree root), not file ids — so
        ``_merge_names`` sees a single uniform shape regardless of which
        entry-generator ran.
        """
        ...
        for other_path, file_id, other_ie, lca_values in walker.iter_all():
            ...
            lca_parent_paths = [_path_dirname(p) for p in lca_paths]
            lca_names = [_path_basename(p) for p in lca_paths]

            kind_winner = self._lca_multi_way(
                (base_ie.kind, lca_kinds), other_ie.kind, this_ie.kind
            )
            parent_winner = self._lca_multi_way(
                (_path_dirname(base_path), lca_parent_paths),
                _path_dirname(other_path),
                _path_dirname(this_path),
            )
            name_winner = self._lca_multi_way(
                (_path_basename(base_path), lca_names),
                _path_basename(other_path),
                _path_basename(this_path),
            )
    ...
    def _merge_names(self, trans_id, paths, parents, names, resolver):
        ...
        winning_parent_path = parents[winning_idx]
        winning_name = names[self.winner_idx[name_winner]]
        if winning_parent_path is not None or winning_name is not None:
            # Pick the tree that owns the winning parent path so we can
            # consult its file-id map (if any) and reuse trans_ids that were
            # allocated during earlier entries in this merge.
            winning_tree = (
                self.base_tree,
                self.other_tree,
                self.this_tree,
