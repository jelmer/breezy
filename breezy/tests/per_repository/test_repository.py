...
class TestRepositoryMakeBranchAndTree(per_repository.TestCaseWithRepository):
    def test_repository_format(self):
        ...
        self.assertIsInstance(
            tree.branch.repository._format, self.repository_format.__class__
        )
...
class TestRepository(per_repository.TestCaseWithRepository):
    ...
    def test_attribute_supports_nesting_repositories(self):
        ...
        self.assertFormatAttribute("supports_nesting_repositories", (True, False))

    def test_attribute_supports_multiple_authors(self):
        """Test the format.supports_multiple_authors."""
        self.assertFormatAttribute("supports_multiple_authors", (True, False))

    def test_attribute_supports_unreferenced_revisions(self):
        """Test the format.supports_unreferenced_revisions."""
        self.assertFormatAttribute("supports_unreferenced_revisions", (True, False))
    ...
    def test_attribute_format_supports_full_versioned_files(self):
        self.assertFormatAttribute("supports_full_versioned_files", (True, False))

    def test_attribute_format_supports_funky_characters(self):
        self.assertFormatAttribute("supports_funky_characters", (True, False))

    def test_attribute_format_supports_leaving_lock(self):
        self.assertFormatAttribute("supports_leaving_lock", (True, False))
    ...
    def test_attribute_format_revision_graph_can_have_wrong_parents(self):
        self.assertFormatAttribute(
            "revision_graph_can_have_wrong_parents", (True, False)
        )
    ...
    def test_attribute_format_supports_setting_revision_ids(self):
        self.assertFormatAttribute("supports_setting_revision_ids", (True, False))

    def test_attribute_format_supports_storing_branch_nick(self):
        self.assertFormatAttribute("supports_storing_branch_nick", (True, False))

    def test_attribute_format_supports_custom_revision_properties(self):
        self.assertFormatAttribute("supports_custom_revision_properties", (True, False))
    ...
    def test_attribute_format_records_per_file_revision(self):
        self.assertFormatAttribute("records_per_file_revision", (True, False))
    ...
    def test_format_initialize_find_open(self):
        ...
        self.assertIsInstance(
            direct_opened_repo._format, self.repository_format.__class__
        )
        # find it via Repository.open
        opened_repo = repository.Repository.open(readonly_t.base)
        self.assertIsInstance(opened_repo, made_repo.__class__)
        self.assertEqual(made_repo._format.__class__, opened_repo._format.__class__)
    ...
    def test_get_revision_delta(self):
        ...
        self.assertEqual(
            [("foo", "file")], [(c.path[1], c.kind[1]) for c in delta.added]
        )
        delta = tree_a.branch.repository.get_revision_delta(rev2)
        self.assertIsInstance(delta, _mod_delta.TreeDelta)
        self.assertEqual(
            [("vla", "file")], [(c.path[1], c.kind[1]) for c in delta.added]
        )
    ...
    def test_iter_revisions(self):
        ...
        self.assertEqual(
            ...
            {(a_rev, a_rev), (b_rev, b_rev), (c_rev, c_rev), (d_rev, None)},
        )
    ...
    def test_pointless_commit(self):
        tree = self.make_branch_and_tree(".")
        self.assertRaises(
            commit.PointlessCommit, tree.commit, "pointless", allow_pointless=False
        )
    ...
    def test_iter_files_bytes(self):
        ...
        extracted = {
            i: b"".join(b)
            for i, b in repository.iter_files_bytes(
                [
    ...
    def test_get_known_graph_ancestry(self):
        ...
        kg = tree.branch.repository.get_known_graph_ancestry([c])
    ...
    def test_add_signature_text(self):
        ...
        rev_a = builder.build_snapshot(None, [("add", ("", None, "directory", None))])
        ...
        if b.repository._format.supports_revision_signatures:
            ...
            self.assertEqual(
                b"This might be a signature", b.repository.get_signature_text(rev_a)
            )
        else:
            b.repository.start_write_group()
    ...
    def test_clone_stacking_policy_upgrades(self):
        ...
        format_description = info.describe_format(
            info_repo.controldir, info_repo, None, None
        )
...
class TestDeltaRevisionFilesFiltered(per_repository.TestCaseWithRepository):
    ...
    def test_multiple_files(self):
        # Test multiple files
        delta = list(
            self.repository.get_revision_deltas(
                [self.repository.get_revision(self.rev1)], specific_files=["foo", "baz"]
            )
        )[0]
        self.assertIsInstance(delta, _mod_delta.TreeDelta)
        self.assertEqual(
    ...
    def test_directory(self):
        ...
        delta = list(
            self.repository.get_revision_deltas(
                [self.repository.get_revision(self.rev1)], specific_files=["bar"]
            )
        )[0]
        self.assertIsInstance(delta, _mod_delta.TreeDelta)
        added = [(c.path[1], c.kind[1]) for c in delta.added]
        # Git repositories don't track empty directories explicitly; the
        # bzr formats list the directory along with its files.
        self.assertIn(
            added,
            (
                [
    ...
    def test_unrelated(self):
        ...
        delta = list(
            self.repository.get_revision_deltas(
                [self.repository.get_revision(self.rev2)], specific_files=["foo"]
            )
        )[0]
        self.assertIsInstance(delta, _mod_delta.TreeDelta)
    ...
    def test_renamed(self):
        ...
        delta2, delta1 = list(
            self.repository.get_revision_deltas(revs, specific_files=["newname"])
        )
    ...
    def test_file_in_directory(self):
        ...
        delta = list(
            self.repository.get_revision_deltas(
                [self.repository.get_revision(self.rev1)], specific_files=["bar/b2"]
            )
        )[0]
        self.assertIsInstance(delta, _mod_delta.TreeDelta)
        added = [(c.path[1], c.kind[1]) for c in delta.added]
        # Git repositories don't track empty directories explicitly; the
        # bzr formats list the parent directory along with the file.
        self.assertIn(
            added,
            (
                [
    ...
    def test_file_in_unchanged_directory(self):
        delta = list(
            self.repository.get_revision_deltas(
                [self.repository.get_revision(self.rev2)], specific_files=["bar/b3"]
            )
        )[0]
        self.assertIsInstance(delta, _mod_delta.TreeDelta)
