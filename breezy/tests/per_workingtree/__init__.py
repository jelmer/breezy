...
def make_scenarios(
    ...
    for workingtree_format in formats:
        result.append(
            (
                workingtree_format.__class__.__name__,
                make_scenario(
    ...
    scenario = make_scenario(remote_server, remote_readonly_server, default_wt_format)
...
class TestCaseWithWorkingTree(per_controldir.TestCaseWithControlDir):
    ...
    def make_branch_builder(self, relpath, format=None):
        ...
        builder = branchbuilder.BranchBuilder(
            self.get_transport(relpath), format=format
        )
        return builder
...
def load_tests(loader, standard_tests, pattern):
    ...
    test_workingtree_implementations = [
        "breezy.tests.per_workingtree.test_" + name for name in test_names
    ]
