...
class TestWorkingTree(TestCaseWithWorkingTree):
    ...
    def test_remove_verbose(self):
        ...
        self.assertEqual(
            ...
            self.apply_redirected(
                None, stdout, stderr, wt.remove, ["hello"], verbose=True
            ),
        )
    ...
    def test_clone_and_commit_preserves_last_revision(self):
        ...
        self.assertNotEqual(
            cloned_dir.open_workingtree().get_parent_ids(), wt.get_parent_ids()
        )

    def test_clone_preserves_content(self):
        wt = self.make_branch_and_tree("source")
        self.build_tree(
            ["added", "deleted", "notadded"],
    ...
    def test_post_build_tree_hook(self):
        ...
        MutableTree.hooks.install_named_hook(
            "post_build_tree", track_post_build_tree, "Test"
        )
        self.workingtree_format.initialize(made_control, revision_id=a)
    ...
    def test_update_turns_local_commit_into_merge(self):
        ...
        self.assertEqual(
            master_tree.branch.last_revision(), tree.branch.last_revision()
        )
    ...
    def test_merge_modified(self):
        ...
        with tree.lock_write():
            ...
            if tree.supports_merge_modified():
                ...
                self.assertRaises(
                    errors.UnsupportedOperation, tree.set_merge_modified, d
                )
                mm = tree.merge_modified()
                self.assertEqual(mm, {})
        if tree.supports_merge_modified():
    ...
    def test_add_conflicts(self):
        ...
        self.assertEqual(
            ConflictList([ContentsConflict("path_a"), TextConflict("path_a")]),
            tree.conflicts(),
        )
    ...
    def test_list_files_versioned_before_ignored(self):
        ...
        self.assertEqual(
            ("foo.pyc", "V", "file"), (files[1][0], files[1][1], files[1][2])
        )
            ...
            expected_results = [
                (
                    ...
                    [
                        ...
                        ("present", "present", "file", present_stat, "file"),
    ...
    def test_filter_unversioned_files(self):
        ...
        paths = [
            "here-and-versioned",
            "here-and-not-versioned",
            "not-here-and-versioned",
            "not-here-and-not-versioned",
        ]
        tree.add(
            ["here-and-versioned", "not-here-and-versioned"], kinds=["file", "file"]
        )
...
class TestFormatAttributes(TestCaseWithWorkingTree):
    def test_versioned_directories(self):
        self.assertSubset(
            [self.workingtree_format.supports_versioned_directories], (True, False)
        )

    def test_supports_setting_file_ids(self):
        self.assertSubset(
            [self.workingtree_format.supports_setting_file_ids], (True, False)
        )

    def test_supports_store_uncommitted(self):
        self.assertSubset(
            [self.workingtree_format.supports_store_uncommitted], (True, False)
        )
...
class TestReferenceLocation(TestCaseWithWorkingTree):
    ...
    def test_set_get_reference_info(self):
        ...
        self.assertEqual(
            ...
            urlutils.join(
                urlutils.strip_segment_parameters(tree.branch.user_url), branch_location
            ),
        )
    ...
    def test_sprout_copies_reference_location(self):
        ...
        self.assertEqual(
            urlutils.join(
                urlutils.strip_segment_parameters(tree.branch.user_url), "../reference"
            ),
            urlutils.join(
                urlutils.strip_segment_parameters(new_tree.branch.user_url),
    ...
    def test_pull_updates_references(self):
        ...
        self.assertEqual(
            urlutils.join(
                urlutils.strip_segment_parameters(new_tree.branch.user_url), "../foo"
            ),
    ...
    def test_merge_updates_references(self):
        ...
        merger = merge.Merger.from_revision_ids(
            tree, orig_tree.branch.last_revision(), other_branch=orig_tree.branch
        )
        merger.merge_type = merge.Merge3Merger
        merger.do_merge()
        self.assertEqual(
            urlutils.join(
                urlutils.strip_segment_parameters(orig_tree.branch.user_url),
                "reference",
            ),
            urlutils.join(
                urlutils.strip_segment_parameters(tree.branch.user_url),
